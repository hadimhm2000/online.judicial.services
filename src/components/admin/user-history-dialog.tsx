...
export default function UserHistoryDialog({ telegramId, fullName, open, onClose }: UserHistoryProps) {
  const [data, setData] = useState<{ summary: Record<string, unknown>; cases: CaseItem[] } | null>(null);
  const [fetchedFor, setFetchedFor] = useState('');

  const loading = open && fetchedFor !== telegramId && data === null;

  useEffect(() => {
    if (!open || !telegramId || fetchedFor === telegramId) return;
    let cancelled = false;
    fetch(`/api/admin/users?telegramId=${telegramId}`)
      .then((r) => r.ok ? r.json() : null)
      .then((d) => { if (!cancelled) { setData(d); setFetchedFor(telegramId); } })
      .catch(() => { if (!cancelled) { setData(null); setFetchedFor(telegramId); } });
    return () => { cancelled = true; };
  }, [open, telegramId, fetchedFor]);
