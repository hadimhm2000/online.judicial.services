...
export async function syncSingleCase(caseId: string): Promise<{
  ...
  try {
    ...
    // Check if case already exists in sheet (find by telegramId + date match in col B and col M)
    ...
    for (let i = 0; i < existingRows.length; i++) {
      ...
      // Match by telegramId (col C = index 2)
      if (row[2] === c.telegramId && row[13] === new Intl.DateTimeFormat('fa-IR', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(c.createdAt))) {
