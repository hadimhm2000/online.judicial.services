...
export async function GET() {
  try {
    ...
    const [completedCases, peakHourData, todayCompleted, todayFailed, allTelegramIds, thisWeekCases, lastWeekCases] = await Promise.all([
      db.case.findMany({
    ...
    // 6. totalActiveUsers
    const totalActiveUsers = allTelegramIds.length;
