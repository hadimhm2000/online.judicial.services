...
EADAH_MADANI_REASONS = [
    ...
    "حكم صادره با حكم ديگري در خصوص همان دعوا و اصحاب آن متضاد است",
    "طرف مقابل درخواست كننده اعاده دادرسي حيله و تقلبي به كار برده",
    "پس از صدور حكم اسنادي به دست آمده كه دليل حقانيت درخواست كننده باشد",
    "حكم به ميزان بيشتر از خواسته صادر شده است",
    "حكم دادگاه مستند به اسنادي بوده كه پس از صدور حكم جعلي بودنشان ثابت شده است",
]
...
EADAH_KIFRI_REASONS = [
    ...
    "تعارض و تضاد مفاد دو حكم بى گناهى",
    "درباره شخصى به اتهام واحد، احكام متفاوتى صادر شود",
    "اثبات جعليت اسناد يا خلاف واقع بودن شهادت گواهان",
    "حدوث واقعه جديد يا كشف ادله جديد بر بى گناهى",
...
async def tn_judge_no_handler(message: Message, state: FSMContext):
    ...
    if message.text == "🔙 بازگشت":
        await message.answer("⚖️ لطفاً نوع دعوی خود را انتخاب فرمایید:",
                             reply_markup=tn_case_type_kb, parse_mode="Markdown")
        await state.set_state(Form.tn_case_type)
        return
