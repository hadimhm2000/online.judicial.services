...
به‌صورت عکس نمایش داده شود، و بین ارسال‌ها تأخیر کوتاهی گذاشته شده تا تلگرام
...
TARGET_IMAGE_KB = 500
# فاصله‌ی زمانی بین ارسال هر صفحه (ثانیه) — از ادغام بصری پیام‌ها در تلگرام جلوگیری می‌کند
...
async def file_tools_receive_pdf(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        for idx, page_path in enumerate(page_paths):
            ...
            # تأخیر کوتاه بین صفحات تا تلگرام آن‌ها را در یک آلبوم بصری ادغام نکند
            if output_count > 1:
